...
const ProjectCard = ({ project }) => {
    ...
    return (
        <div className="relative group overflow-hidden rounded-2xl shadow-lg border border-teal-400 transform transition duration-500">
            ...
            <div className="hidden absolute inset-0 lg:flex flex-col justify-center items-center text-center px-4 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity duration-500">
                <h3 className="text-2xl font-bold text-teal-400 drop-shadow-md mb-2">{project.name}</h3>
                <p className="text-sm text-teal-200 italic mb-3">{project.nameDis}</p>
                <p className="text-xs text-gray-200 mb-4">Full-stack, scalable, and user-focused. Explore for tech details & live demo!</p>
                <button
                    className="bg-teal-600 text-black font-semibold py-2 px-5 rounded-full hover:bg-teal-700 transition-all duration-300"
            ...
            <div className="md:hidden bg-black text-teal-300 p-4 text-center">
                <h3 className="text-xl font-bold">{project.name}</h3>
                <p className="text-sm italic">{project.nameDis}</p>
                <button
                    className="mt-3 bg-teal-600 text-black font-semibold py-2 px-4 rounded-full hover:bg-teal-700 transition"
                    onClick={() => setShowModal(true)}
                >
                    View Details
                </button>
            ...
            {showModal && (
                <div className="fixed inset-0 bg-black/70 flex justify-center items-center z-50 p-4">
                    <div className="bg-white rounded-2xl w-full max-w-3xl p-6 overflow-y-auto max-h-[85vh]">
                        ...
                        <h2 className="text-3xl font-bold text-teal-700 mt-4">{project.name} <span className="text-lg text-gray-500">{project.nameDis}</span></h2>
                        ...
                        <div className="flex flex-wrap gap-4 mt-4 justify-center">
                            <a href={project.liveLink} target="_blank" rel="noopener noreferrer" className="bg-teal-600 text-white py-2 px-6 rounded-lg hover:bg-teal-700 transition">Live Site</a>
                            <a href={project.githubLink} target="_blank" rel="noopener noreferrer" className="border-2 border-teal-600 text-teal-600 py-2 px-6 rounded-lg hover:bg-teal-100 transition">GitHub</a>
                        </div>
                        ...
                        <button onClick={() => setShowModal(false)} className="mt-6 w-full bg-teal-600 text-white py-2 rounded-lg hover:bg-teal-700 transition">Close</button>
                    </div>
                </div>
            )}
        </div>
...
const AllProjectsPage = () => {
    ...
    return (
        <section className="bg-gradient-to-b from-teal-950 to-black text-white py-20 px-4">
            <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-12">
                <aside className="lg:sticky lg:top-20 space-y-6">
                    <h2 className="text-3xl lg:text-4xl font-bold text-teal-200 text-center lg:text-left">Explore Our<br /><span className="text-5xl lg:text-6xl text-teal-500">Latest Projects</span></h2>
                    <p className="text-teal-300 leading-relaxed text-justify lg:text-left">
                    ...
                    <div className="bg-black/40 border border-teal-600 rounded-xl p-4">
                        <h4 className="text-teal-400 text-lg font-semibold mb-2">View by Stack:</h4>
                        <div className="flex gap-4">
                            <button
                                onClick={() => setFilter('vite')}
                                className={`py-1 px-4 rounded-full font-semibold transition ${filter === 'vite' ? 'bg-teal-600 text-black' : 'bg-transparent text-teal-400 border border-teal-500'}`}
                            >
                                Vite Projects
                            </button>
                            <button
                                onClick={() => setFilter('next')}
                                className={`py-1 px-4 rounded-full font-semibold transition ${filter === 'next' ? 'bg-teal-600 text-black' : 'bg-transparent text-teal-400 border border-teal-500'}`}
                            >
                                Next.js Projects
                            </button>
                        </div>
                    ...
                    <div className="flex justify-center lg:justify-start">
                        <Link to='/contact' className="bg-teal-600 hover:bg-black hover:text-teal-500 border-2 border-transparent hover:border-teal-500 text-black font-semibold px-6 py-2 rounded-full transition-all">
                            Get in Touch
                        </Link>
                    </div>
                </aside>
                ...
                <main className="lg:col-span-2 space-y-20">
                    {filter === 'vite' && (
                        <div>
                            <h3 className="text-4xl font-bold text-center text-teal-400 mb-10">Vite Projects</h3>
                    ...
                    {filter === 'next' && (
                        <div>
                            <h3 className="text-4xl font-bold text-center text-teal-400 mb-10">Next.js Projects</h3>
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
